...
export async function saveOAuthTokensRow(p: { provider: string; owner_id?: string | null; access_token: string; refresh_token?: string | null; expiry?: string | null; scope?: string | null }) {
  const { data, error } = await supabaseAdmin.from("oauth_tokens").insert({ provider: p.provider, owner_id: p.owner_id || null, access_token: p.access_token, refresh_token: p.refresh_token || null, expiry: p.expiry || null, scope: p.scope || null }).select().single();
