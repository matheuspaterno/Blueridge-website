...
export async function POST(req: Request) {
  ...
  const instructions = (() => {
    ...
    const shared = `Tone: professional, concise, responsible, approachable. Keep spoken replies short (<= ~15s).\n` +
      ...
      `TOOL PACING: Before calling check_availability, say one complete short sentence such as "Okay, give me one moment while I check that." Finish that sentence, then call the tool. Do not begin saying a date, time, or availability result before the tool returns. Before calling book_appointment, use one complete short sentence such as "One moment while I confirm that." After a tool returns, begin a fresh complete sentence, give the result once, and never restart, repeat, or overlap fragments from the earlier sentence.\n` +
  ...
  `VOICE SPECIFIC: Do not autonomously spell emails or ask for confirmation; rely on user spelling. When confirming a chosen time, state it once clearly. Never output internal markers, JSON, or machine-readable metadata in a spoken or displayed response.\n`;
    return base + shared;
  })();
